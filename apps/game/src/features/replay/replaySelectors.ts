...
export const selectReconstructedStates = createSelector([(_state: ReplayRootShape, data: GameWithMoves | undefined) => data], (data) => (data ? reconstructStates(data) : []))

export const selectTotalPlies = createSelector([selectReconstructedStates], (states) => (states.length > 0 ? states.length - 1 : 0))

export const selectClampedPly = createSelector([selectPly, selectTotalPlies], (ply, totalPlies) => Math.min(Math.max(0, ply), totalPlies))

export const selectCurrentBoard = createSelector([selectClampedPly, selectReconstructedStates], (ply, states) => states[ply] ?? null)

export const selectIsPlaying = createSelector([selectPlaying, selectClampedPly, selectTotalPlies], (playing, ply, totalPlies) => playing && ply < totalPlies)

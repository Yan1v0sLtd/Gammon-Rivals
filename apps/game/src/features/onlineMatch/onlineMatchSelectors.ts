...
export interface CurrentTurnJSON {
...
export interface OnlineMatchRootShape {
...
export const selectMatch = createSelector([selectSnapshot], (snap): MatchRow | null => snap?.match ?? null,);
...
export const selectMoves = createSelector([selectSnapshot], (snap): readonly MoveRow[] => snap?.moves ?? createEmptyArray<MoveRow>(),);

export const selectCurrentGame = createSelector([selectSnapshot], (snap): GameRow | null => snap?.currentGame ?? null,);
...
export const selectCurrentTurn = createSelector([selectMatch], (match): CurrentTurnJSON | null => {
...
const selectDerived = createSelector([selectMoves, selectCurrentTurn], (moves, currentTurn) => deriveState(moves, currentTurn),);

export const selectBoard = createSelector([selectDerived], (d): BoardState => d.board);

export const selectWhoseTurn = createSelector([selectDerived], (d): Player => d.whoseTurn);

export const selectMatchFinished = createSelector([selectMatch], (match) => !!match?.finished_at);

const selectGameFinishedInDb = createSelector([selectCurrentGame], (game) => !!game?.finished_at);
...
export const selectBetweenGames = createSelector([selectGameFinishedInDb, selectMatchFinished], (gameFinished, matchFinished) => gameFinished && !matchFinished,);

// Between games, white always starts the next game (matches edge function expectation).
export const selectEffectiveTurn = createSelector([selectBetweenGames, selectWhoseTurn], (betweenGames, whoseTurn): Player => (betweenGames ? 'white' : whoseTurn),);

export const selectLocalColor = createSelector([selectMatch, (state: OnlineMatchRootShape) => state.auth.userId], (match, userId): Player | null => {
...
export const selectIsLocalTurn = createSelector([selectLocalColor, selectEffectiveTurn], (localColor, effectiveTurn) => localColor !== null && effectiveTurn === localColor,);
...
export const selectInCrawfordGame = createSelector([selectMatch, selectCurrentGame, selectGameFinishedInDb], (match, currentGame, gameFinishedInDb) => !!match && match.crawford_game_number !== null && !!currentGame && match.crawford_game_number === currentGame.game_number && !gameFinishedInDb,);

export const selectGameWinner = createSelector([selectBetweenGames, selectCurrentGame, selectBoard], (betweenGames, currentGame, board): Player | null => betweenGames ? (currentGame?.winner as Player | null) : engineWinner(board),);

export const selectCubeValue = createSelector([selectMatch], (match): CubeValue => (match?.cube_value ?? 1) as CubeValue,);

export const selectCubeOwner = createSelector([selectMatch], (match): Player | null => (match?.cube_owner ?? null) as Player | null,);

export const selectCubeOffer = createSelector([selectMatch], (match): Player | null => (match?.cube_offer ?? null) as Player | null,);
...
const selectRollFirstDie = createSelector([selectCurrentTurn], (t) => t?.dice[0]);
const selectRollSecondDie = createSelector([selectCurrentTurn], (t) => t?.dice[1]);
const selectRollPlayer = createSelector([selectCurrentTurn], (t) => t?.player ?? null);

export const selectRoll = createSelector([selectRollFirstDie, selectRollSecondDie, selectRollPlayer], (firstDie, secondDie, rollPlayer): DiceRoll | null => {
...
export const selectRemaining = createSelector([selectCurrentTurn], (currentTurn): readonly Die[] => (currentTurn?.remaining ?? createEmptyArray<Die>()) as readonly Die[],);

export const selectSelectedFrom = createSelector([(state: OnlineMatchRootShape) => state.onlineMatch.selectedFrom], (selectedFrom) => selectedFrom,);
...
export const selectOpponentPreviewKey = createSelector([selectCurrentTurn, selectIsLocalTurn, selectGameWinner], (currentTurn, isLocalTurn, gameWinner): string | null => currentTurn && !isLocalTurn && currentTurn.remaining.length > 0 && !gameWinner ? [currentTurn.player, currentTurn.dice.join('-'), currentTurn.remaining.join('-'), currentTurn.subMoves.length,].join(':') : null,);

export const selectOpponentPreviewReady = createSelector([selectOpponentPreviewKey, (state: OnlineMatchRootShape) => state.onlineMatch.opponentPreviewReadyKey], (key, readyKey) => key !== null && readyKey === key,);

export const selectLocalLegalMoves = createSelector([selectCurrentTurn, selectIsLocalTurn, selectBoard, selectGameWinner], (currentTurn, isLocalTurn, board, gameWinner): readonly Move[] => {
...
export const selectOpponentLegalMoves = createSelector([selectCurrentTurn, selectIsLocalTurn, selectBoard, selectGameWinner, selectOpponentPreviewReady], (currentTurn, isLocalTurn, board, gameWinner, previewReady): readonly Move[] => {
  if (!currentTurn || isLocalTurn || gameWinner || !previewReady) return createEmptyArray<Move>();
  if (currentTurn.remaining.length === 0) return createEmptyArray<Move>();
  return legalMoves(board, currentTurn.remaining as readonly Die[]);
},);

export const selectLegalOrigins = createSelector([selectLocalLegalMoves], (legal): readonly Position[] => {
...
export const selectValidDestinations = createSelector([selectLocalLegalMoves, selectSelectedFrom], (legal, selectedFrom): readonly Position[] => {
...
export const selectOpponentPreviewDestinations = createSelector([selectOpponentLegalMoves], (legal): readonly Position[] => {
  if (legal.length === 0) return createEmptyArray<Position>();
  const set = new Set<Position>();
...
export const selectCanOfferDouble = createSelector([selectMatchFinished, selectBetweenGames, selectInCrawfordGame, selectMatch, selectCurrentTurn, selectCubeOffer, selectIsLocalTurn, selectCubeValue, selectCubeOwner, selectLocalColor,], (matchFinished, betweenGames, inCrawfordGame, match, currentTurn, cubeOffer, isLocalTurn, cubeValue, cubeOwner, localColor,) => !matchFinished && !betweenGames && !inCrawfordGame && !!match && !!match.opponent_id && currentTurn === null && cubeOffer === null && isLocalTurn && cubeValue < 64 && (cubeOwner === null || cubeOwner === localColor),);
...
export const selectLocalClockBaseMs = createSelector([selectSession, selectRouteMatchId], (session, matchId): number | null => matchId === undefined ? null : Math.max(session.lastLocalActivityMs ?? 0, session.sessionStartedMs),);

export const selectOpponentClockBaseMs = createSelector([selectSession, selectRouteMatchId], (session, matchId): number | null => matchId === undefined ? null : Math.max(session.lastOpponentActivityMs ?? 0, session.sessionStartedMs),);
...
export const selectTurnDisplayDeadlineMs = createSelector([(state: OnlineMatchRootShape) => state.onlineMatch.turnSeconds, selectLocalClockBaseMs], (turnSeconds, localClockBase): number | null => turnSeconds === null || localClockBase === null ? null : localClockBase + turnSeconds * 1000,);

export const selectTurnDeadlineMs = createSelector([(state: OnlineMatchRootShape) => state.onlineMatch.turnSeconds, selectLocalClockBaseMs, selectMatchFinished, selectBetweenGames, selectIsLocalTurn,], (turnSeconds, localClockBase, matchFinished, betweenGames, isLocalTurn): number | null => {
...
export const selectInactivityDeadlineMs = createSelector([selectOpponentClockBaseMs, (state: OnlineMatchRootShape) => state.onlineMatch.inactivityForfeitMs,], (opponentClockBase, inactivityForfeitMs): number | null => opponentClockBase === null ? null : opponentClockBase + inactivityForfeitMs,);

export const selectPresenceForfeitDeadlineMs = createSelector([(state: OnlineMatchRootShape) => state.onlineMatch.opponentDisconnectedAt, selectRouteMatchId,], (opponentDisconnectedAt, matchId): number | null => {
...
export const selectCanClaimByInactivity = createSelector([selectMatchFinished, selectMatch, (state: OnlineMatchRootShape) => state.onlineMatch.inactivityClaimAvailable,], (matchFinished, match, inactivityClaimAvailable): boolean => !matchFinished && !!match?.opponent_id && inactivityClaimAvailable,);
...
export const selectOpponentActivitySignature = createSelector([selectMoves, selectCurrentTurn, selectLocalColor], (moves, currentTurn, localColor): string => {
  ...
  const oppTurnPart = currentTurn && currentTurn.player === oppositeColor ? `${currentTurn.dice.join('-')}:${currentTurn.subMoves.length}:${currentTurn.remaining.length}` : '';
...
export const selectBotPokeKey = createSelector([selectMatch, selectMatchFinished, selectBetweenGames, selectEffectiveTurn, selectCurrentTurn, selectMoves,], (match, matchFinished, betweenGames, effectiveTurn, currentTurn, moves): string | null => {
...
// intended behaviour of a run-out timer.
export const selectAutoActionKind = createSelector([selectCanRoll, selectCurrentTurn], (canRoll, currentTurn): AutoActionKind | null => canRoll ? 'roll' : currentTurn ? 'force-end' : null,);

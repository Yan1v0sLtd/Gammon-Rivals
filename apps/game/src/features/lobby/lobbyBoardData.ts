...
export type LobbyBoard = {readonly id: LobbyBoardId, readonly name: string, readonly subtitle: string, readonly image: string, readonly holderImage: string | null, readonly accent: string, readonly background: string, readonly backgroundTone: string, readonly unlockLevel: number, readonly priceGems: number}
...
function normalizePublicAssetPath(path: string | null | undefined): string | undefined {
  const trimmed = path?.trim(); if (!trimmed) return undefined; if (/^(https?:|data:|blob:)/i.test(trimmed)) return trimmed; return trimmed.startsWith("/") ? trimmed : `/${trimmed}`
}
export function lobbyBoardFromConfig(config: BoardThemeConfigRow): LobbyBoard {
  const accent = metadataText(config.metadata, "accent") ?? "#ffd35d"; return {id: config.id, name: config.display_name, subtitle: metadataText(config.metadata, "subtitle") ?? `Unlocks at level ${config.unlock_level}`, image: normalizePublicAssetPath(config.preview_image) ?? "/lobby/board-previews/classic-green.webp", holderImage: normalizePublicAssetPath(config.holder_image) ?? null, accent, background: normalizePublicAssetPath(config.lobby_background_image) ?? "/lobby/backgrounds/classic-green.webp", backgroundTone: metadataText(config.metadata, "backgroundTone") ?? toneForAccent(accent), unlockLevel: config.unlock_level, priceGems: config.price_gems ?? 0}
}

 ...
 */
export type LobbyModal =
  | { kind: 'none' }
  | { kind: 'difficulty'; error: string | null }
  | { kind: 'boardPurchase'; boardId: string; error: string | null }
  | { kind: 'dailyBonus'; error: string | null; justClaimed: ClaimedDailyReward | null }
  | { kind: 'wheel' }
  | { kind: 'howToPlay' }
  | { kind: 'missions' };
...
export type MatchmakingState =
  ...
  | {
      ...
      target: number;
      turnSeconds: number;
      // 'pvp' | 'online' | 'easy' | 'medium' | 'hard' — plain string from the server RPCs.
      mode: string;
    };
...
export const lobbySlice = createSlice({
  ...
  reducers: {
    ...
    difficultyModalOpened(state) {
      state.modal = { kind: 'difficulty', error: null };
    },
    boardPurchaseModalOpened(state, action: PayloadAction<string>) {
      state.modal = { kind: 'boardPurchase', boardId: action.payload, error: null };
    },
    dailyBonusModalOpened(state) {
      state.modal = { kind: 'dailyBonus', error: null, justClaimed: null };
    },
    ...
    matchmakingTicked(state, action: PayloadAction<MatchmakingTickedPayload>) {
      ...
      state.matchmaking.elapsedSeconds = Math.min(
        state.matchmaking.maxSeconds,
        Math.max(0, action.payload.elapsedSeconds)
      );
    },

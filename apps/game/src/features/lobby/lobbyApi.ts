...
export const lobbyApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    getUserBoardInventory: build.query<readonly string[], string>({
      ...
      providesTags: (_result, _error, userId) => [{ type: 'BoardInventory', id: userId }],
    }),
    ...
    getDailyBonusState: build.query<UserDailyBonusRow | null, string>({
      ...
      providesTags: (_result, _error, userId) => [{ type: 'DailyBonusState', id: userId }],
    }),
    getWheelState: build.query<WheelState | null, string>({
      ...
      providesTags: (_result, _error, configId) => [{ type: 'WheelState', id: configId }],
      ...
      async onCacheEntryAdded(configId, { cacheDataLoaded, cacheEntryRemoved, dispatch, getCacheEntry }) {
        try {
          ...
          for (;;) {
            ...
            const ticked = new Promise<'tick'>((resolve) => {
              timerId = window.setTimeout(() => resolve('tick'), delayMs);
            });
            ...
            dispatch(lobbyApi.util.invalidateTags([{ type: 'WheelState', id: configId }]));
          }
        } catch {
    ...
    getDailyMissions: build.query<MissionsState | null, string>({
      ...
      providesTags: (_result, _error, userId) => [{ type: 'DailyMissions', id: userId }],
      ...
      async onCacheEntryAdded(userId, { cacheDataLoaded, cacheEntryRemoved, dispatch }) {
        ...
        try {
          await cacheDataLoaded;
          const invalidate = () => {
            dispatch(lobbyApi.util.invalidateTags([{ type: 'DailyMissions', id: userId }]));
          };
          channel = supabase
            .channel(`missions:${userId}`)
            .on(
              'postgres_changes',
              { event: '*', schema: 'public', table: 'player_daily_missions', filter: `profile_id=eq.${userId}` },
              invalidate,
            )
            .on(
              'postgres_changes',
              { event: '*', schema: 'public', table: 'player_weekly_pass', filter: `profile_id=eq.${userId}` },
              invalidate,
            )
            .on(
              'postgres_changes',
              { event: '*', schema: 'public', table: 'player_streak', filter: `profile_id=eq.${userId}` },
              invalidate,
            )
            .subscribe();
    ...
    getTableConfigs: build.query<readonly TableConfigRow[], TableConfigRow['kind']>({
      ...
      providesTags: (_result, _error, kind) => [{ type: 'TableConfigs', id: kind }],
      keepUnusedDataFor: 1800,
    }),
    ...
    claimDailyBonus: build.mutation<ClaimDailyBonusResult | null, { userId: string }>({
      ...
      invalidatesTags: (_result, _error, { userId }) => [{ type: 'DailyBonusState', id: userId }],
    ...
    claimMission: build.mutation<ClaimMissionResult | null, { missionId: string; userId: string }>({
      ...
      invalidatesTags: (_result, _error, { userId }) => [{ type: 'DailyMissions', id: userId }],
    }),

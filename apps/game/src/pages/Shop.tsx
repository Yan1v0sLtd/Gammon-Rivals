...
function XpBadge({className = ''}: { className?: string }) {
  return (<svg viewBox="0 0 100 110" className={`w-auto drop-shadow-[0_2px_3px_rgba(0,0,0,0.4)] ${className}`}
               aria-hidden="true">
    <defs>
...
function DiceIcon({className = ''}: { className?: string }) {
  return (<svg viewBox="0 0 100 80" className={`w-auto drop-shadow-[0_2px_3px_rgba(0,0,0,0.4)] ${className}`}
               aria-hidden="true">
    <defs>
  ...
  return (<article
    ...
    <div className="flex flex-1 flex-col p-5">
      ...
      <div className="flex flex-wrap items-stretch justify-center gap-4 border-b border-white/10 py-6">
        {bundle.rewards.slice(0, 4).map((r, i) => (<div key={i}
...
function ShopSkeleton() {
  return (<div className="relative z-[3] grid grid-cols-[340px_1fr] gap-8 p-10" aria-hidden="true">
...
export function ShopModal({onClose}: { readonly onClose: () => void }) {
  ...
  const handleUsdPurchase = async (item: BuyDescriptor) => {
    ...
    if (outcome.status !== 'granted') {
      if (outcome.status === 'error') {
        const {code} = outcome;
        if (code === 'already_owned') showToast('info', 'You already own that board.'); else if (code === 'unsupported_grant') showToast('error', `${item.label}: unsupported grant.`); else if (code === 'not_authorized') showToast('info', 'Purchases are available in the app.'); else showToast('error', 'Purchase failed.');
      }
  ...
  const buyWithGems = async (item: BuyDescriptor) => {
    ...
    try {
      ...
      if (msg.includes('unsupported_grant')) showToast('info', `${item.label} — coming soon`); else if (msg.includes('insufficient_gems')) {
      ...
      else if (msg.includes('already_owned_board')) showToast('info', 'You already own that board.'); else if (msg.includes('purchase_limit_reached')) showToast('info', 'Purchase limit reached for this item.'); else showToast('error', 'Purchase failed. Try again.');
      return;
    }
    setBusyId(null);
    if (sourceEl && item.flightKind) spawnFlights(item.flightKind, sourceEl, 6);
  ...
  return (<>
        ...
        >
          ...
          <header
            ...
            <div className="flex items-center justify-end gap-4">
              ...
              >
                <CurrencyPill flyTarget="coins" label="Coins" value={wallet?.coins} icon="/lobby/icons/gold-coin.webp"
                              onAdd={() => {
                              }} showAdd={false}/>
                <CurrencyPill flyTarget="gems" label="Gems" value={wallet?.gems} icon="/lobby/icons/gem.webp"
                              onAdd={() => {
                              }} showAdd={false}/>
              </div>
    ...
    {rewardFlights.map((spec) => (<RewardFlight key={spec.id} spec={spec} onLanded={removeFlight}/>))}

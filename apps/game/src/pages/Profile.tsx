...
function Stat({
  ...
  return (<div className={`profile-stat-card ${wide ? 'profile-stat-card--wide' : ''}`}>
    {realIcon ? (<span className="grid h-9 w-9 shrink-0 place-items-center" aria-hidden="true">
          <img
            src={realIcon}
            alt=""
            draggable={false}
          ...
          />
        </span>) : (<span className={`profile-stat-icon profile-stat-icon--${icon}`} aria-hidden="true">
          <span/>
        </span>)}

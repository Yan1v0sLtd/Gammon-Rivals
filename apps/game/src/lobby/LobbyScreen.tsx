...
import {useGetDailyBonusConfigsQuery, useGetDailyBonusStateQuery, useGetDailyMissionsQuery, useGetLobbyBoardsQuery, useGetUserBoardInventoryQuery} from '../features/lobby/lobbyApi';
import {computeBoardState, computeDaysClaimedInCurrentStreak, computeUpcomingDay, selectLobbyBoards, selectOwnedBoardIds, selectSelectedLobbyBoard, todayET} from '../features/lobby/lobbySelectors';
...
const LOBBY_STATIC_ASSETS: readonly string[] = ['/lobby/carousel/gem.webp', '/lobby/holders/royal-holder.webp', '/lobby/icons/friends.webp', '/lobby/icons/gem.webp', '/lobby/icons/gold-coin.webp', '/lobby/icons/online-players.webp', '/lobby/icons/rewards-gift.webp', '/lobby/icons/settings-gear.webp', '/lobby/icons/trophy.webp', '/lobby/cards/coins-offer.webp', '/lobby/cards/daily-bonus.webp', '/lobby/nav/events.webp', '/lobby/nav/missions.webp', '/lobby/nav/nav-bg.webp', '/lobby/nav/tournaments.webp', '/lobby/nav/vip-club.webp',];
...
const LOBBY_SECONDARY_ASSETS: readonly string[] = ['/lobby/difficulties/beginner.webp', '/lobby/difficulties/advanced.webp', '/lobby/difficulties/pro.webp', '/lobby/difficulties/expert.webp', '/lobby/difficulties/grand-master.webp', '/lobby/cards/how-to-play-popup.webp',];
...
export function LobbyScreen() {
  ...
  const boardsQuery = useGetLobbyBoardsQuery(undefined, {skip: !isSupabaseConfigured});
  ...
  const {effectiveSelectedBoardId, selectedBoard} = selectedLobbyBoard;
  const boardsLoading = boardsQuery.isLoading || (isSupabaseConfigured && boardsQuery.isUninitialized);
  const {
    flights,
    spawnFlights,
    removeFlight
  ...
  const dailyBonus = {
    ...
    isLoading: dailyBonusConfigsQuery.isLoading || dailyBonusStateQuery.isLoading,
  };
  ...
  const missionsClaimableBadge = missionsQuery.data?.missions.filter((m) => m.completed_at && !m.claimed_at).length ?? 0;
  ...
  const [tourDismissed, setTourDismissed] = useState(() => typeof localStorage !== 'undefined' && localStorage.getItem('gr_tutorial_done') === '1');
  const tourPending = !!userId && !!profile && profile.tutorial_completed_at == null && !tourDismissed;
  ...
  const boardStateOf = (board: LobbyBoard) => computeBoardState({
    boardId: board.id,
  ...
  useEffect(() => {
    ...
    if (!userId || !dailyBonus.canClaim || dailyBonus.isLoading) return;
  ...
  // recover it via the board-theme adapter and never fall through to the generic
  // placeholder board.
  useEffect(() => {
  ...
  return (<main className="lobby-screen relative min-h-dvh overflow-x-hidden bg-[#071120] text-white">
    ...
    {fadingBoard && fadingBoard.id !== selectedBoard?.id ? (
      <LobbyBackgroundLayer key={`bg-${fadingBoard.id}`} board={fadingBoard} mode="leaving"/>) : null}
    {selectedBoard ? (<LobbyBackgroundLayer

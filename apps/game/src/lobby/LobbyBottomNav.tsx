...
export function LobbyBottomNav({
  ...
  return (<nav aria-label="Lobby sections" className="lobby-bottom-nav-shell">
    ...
    <div className="lobby-bottom-nav-row">
      {lobbyNavItems.map((item) => {
        ...
        if (locked && item.image) {
          return (<div key={item.id} className="lobby-bottom-nav-slot is-locked">
            ...
            <NavFeatureLock
              ...
              onToggle={() => setOpenLockKey((k) => (k === item.id ? null : item.id))}
            />
          </div>);
        }
        ...
        </button>) : (<span key={item.id} className="lobby-bottom-nav-slot is-placeholder" aria-hidden="true"/>);
      })}
    </div>
  </nav>);
}

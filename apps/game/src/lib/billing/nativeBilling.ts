// Native Play Billing (Android only) via cordova-plugin-purchase.
//
// Loaded only through getBilling()'s native branch with a dynamic import(),
// so neither this module nor the plugin runtime ever enters the web bundle.
//
// Flow: order() → Play purchase dialog → token → store.validator POSTs
// { shopItemId, purchaseToken } (+ buyer JWT) to the validate-google-purchase
// edge fn, which verifies the token with Google and grants via
// fulfill_google_purchase → on `granted` we finish() and resolve purchase().
//
// The SKU list is DATA — read from shop_items.google_product_id, never a
// hardcoded map (that map drifted and caused the collision bug).
//
// ⚠️ The plugin is typed loosely (`any`) on purpose: v13's receipt/event

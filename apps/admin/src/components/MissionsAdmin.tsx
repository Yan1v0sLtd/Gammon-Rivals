        ...
        <table className="w-full text-sm">
          ...
          >
            ...
            <td className="px-3 py-2 text-right font-mono text-white/80">
              {t.resolution_mode === 'fixed' ? t.goal_value : t.resolution_mode === 'stretch' ? `×${t.stretch_factor} [${t.goal_min}..${t.goal_max}]` : 'auto'}
            </td>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <Field label="Mission type">
          ...
          >
            ...
            {types.map((t) => (<option key={t.mission_type} value={t.mission_type}>
        ...
        {draft.resolution_mode === 'personalized' ? (<Field label="Goal & reward" wide>
          <div
      ...
      {editing === r.mission_type && draft && (<div className="mt-3 space-y-3 border-t border-white/10 pt-3">
        ...
        {draft.supports_personalized && (<div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {COEFFICIENT_FIELDS.map((f) => (<label key={String(f.key)} className="block">
                        <span
...
function RerollEditor({canManage}: { readonly canManage: boolean }) {
  ...
  return (<div className="max-w-xl rounded-xl border border-white/10 bg-white/[0.045] p-4">
    ...
    <div className="space-y-2">
      {ladder.map((cost, i) => (<div key={i} className="flex items-center gap-2">
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.045] p-3">
        ...
        <div className="mb-2 grid grid-cols-3 gap-2">
          {[{
            ...
            value: whales,
            set: setWhales
          },].map((row) => (<label key={row.label} className="text-xs text-white/60">
            {row.label}
            <input
      ...
      {!state ? (<p className="text-sm text-white/50">Select a test user, or create one to begin.</p>) : (<>
        ...
        <div>
          ...
          {state.missions.length === 0 ? (
            ...
            <div className="space-y-2">
              {state.missions.map((m) => (<div key={m.id} className="rounded bg-black/30 p-2 ring-1 ring-white/5">
                <div className="flex items-center justify-between">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-white/60">
                  ...
                  {m.rewards.length > 0 && (<span>
                            {m.rewards.map((r, i) => (<span key={i} className="ml-1 rounded bg-white/10 px-1.5 py-0.5">

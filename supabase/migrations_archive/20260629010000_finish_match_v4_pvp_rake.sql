-- finish_match v4 — PvP winner prize derived from pot, rake, and lose-prize.
--
-- Background: v3 paid PvP winners cfg.prize_coins and PvP losers
-- cfg.prize_coins_loss, but those columns were tuned for vs-AI
-- RTP (the house pays out a fraction of stake assuming a win rate).
-- In PvP the "house" is the pot — players each stake entry_fee_coins
-- and the house only takes a rake. Using the vs-AI numbers in PvP
-- led to non-uniform implicit rake per tier (Beginner ~20%, GM
-- actually NEGATIVE -5% because the seeded numbers paid out more
-- than the pot).
--
-- v4 makes the PvP math explicit:
--
--   pot       = 2 * cfg.entry_fee_coins
--   rake      = pot * cfg.pvp_rake_pct / 100         (house keeps)
--   loser     = cfg.prize_coins_loss                  (same as vs-AI)
--   winner    = pot - rake - loser                    (whatever's left)
--
-- Both players get coins on every non-abandoned match. The
-- "consolation" lose-prize is unchanged from vs-AI — players who
-- bounce between the two modes see the same loss feel.
--
-- vs-AI path is UNTOUCHED: it still pays cfg.prize_coins on a win
-- and cfg.prize_coins_loss on a loss, with the risk-free refund
-- for the first 10 difficulty matches.
--
-- Abandonment behaviour unchanged: abandoner gets 0, the other side
-- gets the regular winner prize, ELO updates regardless.
...
declare
  ...
  -- PvP rake math working set (filled only when is_pvp = true):
  pvp_pot int := 0;
  pvp_rake int := 0;
...
begin
  ...
  -- ------------------------------------------------------------------
  -- Pre-compute the PvP rake split once. Used by both the owner-side
  -- and opponent-side payout blocks below. Done here (not inside each
  -- block) so the math lives in one place — change pvp_rake_pct in
  -- the BO and both sides update together.
  -- ------------------------------------------------------------------
  if cfg.id is not null and is_pvp then
    ...
    if pvp_winner_coins < 0 then
      -- Defensive guard: if lose-prize + rake exceed the pot (would
      -- require a misconfigured tier), clamp the winner payout to 0
      -- rather than emitting a negative coin grant. The BO RTP
      -- dashboard will surface this as a misconfiguration.
      pvp_winner_coins := 0;
    end if;
  end if;
  ...
  if cfg.id is not null then
    if owner_won then
      ...
      -- Abandoner: no payout at all. Skip the risk-free upgrade.
      coins_awarded := 0;
    else
      if is_pvp then
    ...
    -- ----------------------------------------------------------------
    -- 2. Opponent-side payout (PvP only).
    -- ----------------------------------------------------------------
    if is_pvp then
      declare

-- Controller ease fix (found via simulation). The ease branch computed
-- ceil(prior * 0.75), which for goal 3 rounds back UP to 3 — so a repeated-misser
-- never descended below 3 (stranded at a goal they keep failing, defeating the
-- anti-frustration design). Now it rounds DOWN and is forced at least one step
-- below prior, so it descends to the floor (3->2->1) and rests where it's winnable.
-- Ramp / cold-start / cap unchanged. Affects all personalized types.
-- Simulated 10-day trajectory (baseline 2): struggler was 3,3,3,3,... now 3,3,2,2,1,1,...
...
begin
  ...
  if v_has_ctrl is null then
    ...
    if v_misses >= cfg.ease_after then
      -- ease rounds DOWN and is forced >= one step below prior, so repeated
      -- misses descend to the floor (was ceil -> stranded at the same goal)
      v_target := least(

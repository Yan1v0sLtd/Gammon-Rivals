-- Generalize the personalized generator into mp_assign_personalized(profile, type)
-- and add the wager_coins type. Adds goal_round_to so the shared controller works
-- for both count goals (round 1) and coin-magnitude goals (round 250). Behavior of
-- play_matches is unchanged (validated). wager_coins ships DARK (template disabled,
-- not in any cron).

alter table public.mission_type_config
  add column if not exists goal_round_to int not null default 1;
alter table public.mission_type_config
  drop constraint if exists mtc_goal_round_ck;
alter table public.mission_type_config
  add constraint mtc_goal_round_ck check (goal_round_to >= 1);

-- wager_coins: personalized, magnitude-scaled goal (round + ramp in 250-coin units)
update public.mission_type_config
  set supports_personalized = true, up_step = 250, goal_round_to = 250
...
select 'wager_coins','coins_wagered_per_day','common','personalized','daily',
       'Wager {goal} coins','Put your coins in play.', 10, false
where not exists (select 1 from public.mission_templates
  where mission_type='wager_coins' and resolution_mode='personalized');
...
begin
  ...
  if p_mission_type = 'play_matches' then
    ...
    v_focus_tier := null;  -- tier-agnostic: all coin wagering counts
...
-- mp_assign_play_matches becomes a thin wrapper (cron / test calls keep working)
create or replace function public.mp_assign_play_matches(p_profile_id uuid)
returns public.player_daily_missions
language sql security definer set search_path = public, pg_temp
as $$ select public.mp_assign_personalized(p_profile_id, 'play_matches'); $$;

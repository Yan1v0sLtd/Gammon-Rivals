-- win_streak baseline is now DYNAMIC: the player's observed average win-streak
-- (avg length of consecutive-win runs over the window, gaps-and-islands over the
-- ordered match sequence — cross-day, like the live counter — floored at 2), instead
-- of a flat 2. Goal now scales with skill (e.g. avg streak 2/3/5 → goal 2/3/5).
-- floor_mult dropped 1.0 -> 0.5 so a strong player has easing room (floor 1.0 would
-- trap a whale at "win 5"). Other types unchanged.
update public.mission_type_config set floor_mult = 0.5 where mission_type = 'win_streak';
...
begin
  ...
  if p_mission_type = 'play_matches' then
    ...
    v_focus_tier := null;
    v_loss_unit  := v_fee * (1 - v_rtp/100.0);
    -- baseline = avg length of the player's consecutive-win runs (observed streak
    -- skill), floored at 2. Gaps-and-islands over the ordered match sequence.

-- Daily missions: make a PARTIAL re-run (backfill) avoid duplicate types too.
--
-- 20260720 made the per-slot pick always fill, preferring a distinct mission_type
-- via ORDER BY. But that preference reads picked_types, which is only populated as
-- the loop INSERTs slots this run. On a re-run where common/rare are already
-- filled (the slot-count check `continue`s them), picked_types stays empty when we
-- reach the epic slot — so the epic can collide with an existing common's type
-- (e.g. an epic win_coins_net next to the common win_coins_net).
--
-- Fix: seed picked_ids/picked_types from the player's already-active daily
-- missions before the loop, so the distinct-type/id preference is honoured against
-- what they already hold. Pure refinement on top of 20260720 — fresh assignments
-- are unaffected (nothing active yet to seed from).
...
begin
  ...
  -- Seed the picked lists from already-active daily missions (incl. any just
  -- added by the personalized loop) so a backfill respects what's already held.
  select

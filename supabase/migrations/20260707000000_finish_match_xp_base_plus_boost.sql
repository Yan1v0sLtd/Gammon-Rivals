-- finish_match: XP fix — base + difficulty boost, granted on win AND loss.
--
-- (a) XP is granted on EVERY completed match (win OR loss); abandoning forfeits it.
-- (b) BUG FIX: XP was `base_xp_win * xp_multiplier_pct / 100` (= base × boost%),
--     which makes Beginner (0% boost) grant 0 XP. The intended formula adds the
--     boost on top of the base:
--         base_xp_win * (100 + xp_multiplier_pct) / 100   (= base × (1 + boost%))
--     so at base_xp_win = 50:
--         Beginner 50 · Advanced 75 · Pro 125 · Expert 175 · Grand Master 300
--     (× any active XP-boost item via current_xp_multiplier).
--
-- Coins / risk-free first-10 / PvP rake / ELO / return payload are unchanged.
...
begin
  ...
  if cfg.id is not null then
    -- XP on every completed match (win OR loss), base + boost% (Beginner = base, not 0).
    xp_mult := public.current_xp_multiplier(match_row.owner_id);
    if not p_owner_abandoned then
    ...
    if is_pvp then
      ...
      begin
        ...
        if not p_opponent_abandoned then

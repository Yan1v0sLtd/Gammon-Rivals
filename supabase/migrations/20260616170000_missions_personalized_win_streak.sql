-- win_streak personalized type. The operator had already flipped supports_personalized
-- on and set floor_reward=100 in the BO — we PRESERVE floor_reward (do not touch it)
-- and only add the anti-strand tuning + the generator branch + a disabled template.
-- Anti-strand: flat baseline 2 with base_stretch 1.0 + floor_mult 1.0 → cold-start
-- "win 2", floor "win 2", cap "win 4". Winning isn't pure effort, so the floor must
-- be genuinely reachable (the ease controller anchors to last-completed and can't
-- drop a never-completed goal). win-based reward is a bounded faucet (stakes-capped),
-- NOT self-funding — monitor on the RTP dashboard. Ships DARK (template disabled,
-- rollout_pct 0).
update public.mission_type_config
  set supports_personalized = true,
      base_stretch = 1.0, floor_mult = 1.0, cap_mult = 2.0,
      up_step = 1, goal_round_to = 1
  where mission_type = 'win_streak';   -- NB: floor_reward left as operator set it (100)
...
select 'win_streak','win_streak','common','personalized','daily',
       'Win {goal} in a row','Stack up the wins.', 10, false
where not exists (select 1 from public.mission_templates
  where mission_type='win_streak' and resolution_mode='personalized');
...
declare
  ...
  v_baseline numeric; v_loss_unit numeric;
...
begin
  ...
  if p_mission_type = 'play_matches' then
    ...
    v_focus_tier := null;                          -- global streak (any tier)
    v_loss_unit  := v_fee * (1 - v_rtp/100.0);     -- bounded faucet: stakes per match in the streak
    v_baseline   := 2;                             -- flat default; controller personalizes from here
  else
  ...
  if v_has_ctrl is null then
    ...
    if v_misses >= cfg.ease_after then
      v_target := (ceil(coalesce(v_last_completed, v_prior.resolved_goal) * cfg.ease_factor / v_round) * v_round)::int;
      v_misses := 0;
    else
  ...
  v_reward := greatest(cfg.floor_reward,
    (round((cfg.reward_pct * v_target * v_loss_unit) / cfg.round_to) * cfg.round_to)::int);
  ...
  values (p_profile_id, v_template.id, 'common', v_target, 0, v_day_end, now(), 'daily', v_focus_tier, v_reward)

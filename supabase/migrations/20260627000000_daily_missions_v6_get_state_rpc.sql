-- Phase 6 backend: single-round-trip aggregator the frontend calls
-- on lobby render. Returns a fully-formed jsonb shape with active
-- missions (+ rewards bundles), weekly pass state, all chest
-- milestones (+ their bundles), streak state, streak chest bundle,
-- and reroll pricing/usage. Avoids 5-7 sequential round-trips on
-- the slow path of opening the missions modal.
...
comment on function public.get_player_missions_today() is
  'Frontend aggregator: single round-trip returning active missions, weekly pass state, chest milestones, streak state, streak chest bundle, and reroll pricing/usage.';

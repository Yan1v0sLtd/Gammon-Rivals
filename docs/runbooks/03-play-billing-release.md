...
> Status: pending · Last verified: 2026-07-02 · Owner: developer

## Purpose

Finish the Google Play Billing native wiring and deploy path. Everything except
the on-device plugin glue is built. This runbook finishes it once the Play
account is verified, the app exists, and we have an AAB on an Internal testing
track.
...
- Internal testing track with our AAB (built below) + your email as a **license
  tester** (Setup → License testing). Test-license buys do not charge.
...
on the transaction lifecycle. Then flip the `getBilling()` factory's native
branch to `await import('./nativeBilling')`.
...
const SKU_TO_ITEM: Record<string, string> = {
  small_coins: "small-coins",
  starter_bundle: "starter-bundle",
  xp_boost_7d: "xp-boost-7d",
  coin_stack: "coin-stack",
  bowl_of_gems: "bowl-of-gems",
  mega_bundle: "mega-bundle",
};

store.register(
  Object.keys(SKU_TO_ITEM).map((id) => ({
    id,
...
`capacitor.config.ts` sets `webDir: 'dist/play'` and has no `server.url` (the
game is Capacitor-only and `/play` is not web-served).
...
Install via the testing link on a device signed in as a license tester → open
Shop → Buy → Google's test purchase dialog → token → our edge fn validates →
gems/coins land. `purchases` row recorded as `provider=google`. Re-buying the
same token is a no-op (idempotent). Then build P4 (refund handling via
Real-time Developer Notifications). Normal dev continues on the bundled
`dist/play`: `pnpm run android:sync` + rebuild/reinstall — no remote `server.url`
workflow remains.

## Verification

- One end-to-end purchase from store dialog to granted wallet value.
- Duplicate-token check and cancellation check behave correctly.
- `purchases` row recorded as `provider=google`.

## Troubleshooting

- The client key env var is **`VITE_SUPABASE_PUBLISHABLE_KEY`**, not
  `VITE_SUPABASE_ANON_KEY`.
- `handleUsdPurchase` is still admin-gated, so the first on-device test buy is
  done as an admin (regular-user un-gating is a later launch step).

## Related documents

- `docs/reference/05-economy-reference.md` — billing grants and validation.
- `docs/runbooks/02-android-build.md` — building the APK/AAB.

...
function indepSingleMoves(state: BoardState, die: Die): Array<{ from: number | typeof BAR; to: number | typeof OFF }> {
  const p = state.turn;
  ...
  const out: Array<{ from: number | typeof BAR; to: number | typeof OFF }> = [];
    ...
    if (target >= 0 && target < 24) {
      ...
      if (p === 'white') {
        if (target === 24) out.push({ from, to: OFF });
          ...
          let blocked = false;
          for (let i = 18; i < from; i++) { const q = state.points[i]!; if (q.owner === 'white' && q.count > 0) { blocked = true; break; } }
        ...
        if (target === -1) out.push({ from, to: OFF });
          ...
          let blocked = false;
          for (let i = from + 1; i <= 5; i++) { const q = state.points[i]!; if (q.owner === 'black' && q.count > 0) { blocked = true; break; } }
          if (!blocked) out.push({ from, to: OFF });
        }
      }
    }
...
describe('engine fuzz — random full games hold all invariants', () => {
  it('plays 120 seeded games to completion with no invariant violation', () => {
    ...
    for (let seed = 1; seed <= GAMES; seed++) {
      ...
      expect(state.off[w as Player]).toBe(15);
      totalTurns += turns;
    }
